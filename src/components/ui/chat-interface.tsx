      ...
      <AnimatePresence mode="wait">
        {shouldShowAvatar ? (
          <motion.img
            ...
            src={person.avatar}
            alt={person.name}
            className="w-8 h-8 rounded-full object-cover flex-shrink-0 border-[1.5px] border-white/20"
      ...
      >
        {!isContinuation && (
          <motion.div
            ...
            style={{ color: chatStyle.nameColor || '#9ca3af' }}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
...
const ChatComponent: React.FC<ChatComponentProps> = ({ config, uiConfig = {} }) => {
  ...
  const scrollToBottom = useCallback(() => {
    // Scroll only within the chat container — never touches the page scroll
  ...
  const gradientBackground = useMemo(() => {
    const bg = ui.backgroundColor;
    if (!bg.startsWith('#') || bg.length < 7) return 'none';
    return `linear-gradient(to bottom, ${hexToRgba(bg, 1)} 0%, ${hexToRgba(bg, 0.95)} 20%, ${hexToRgba(bg, 0.8)} 40%, ${hexToRgba(bg, 0.4)} 70%, ${hexToRgba(bg, 0)} 100%)`;
  }, [ui.backgroundColor]);
  ...
  return (
    ...
    >
      ...
      <div
        ref={containerRef}
        className="d2d-chat-noscroll p-8 overflow-y-scroll h-full"
        style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' } as React.CSSProperties}
      >
        <style>{`.d2d-chat-noscroll::-webkit-scrollbar{display:none}`}</style>
